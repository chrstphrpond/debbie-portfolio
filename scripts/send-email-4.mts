...
import * as BroadcastModule from "../emails/broadcast-4.tsx";
...
const SUBJECT = "This Is Our Moment — Please Vote";
...
const previewEmail = PREVIEW_ARG ? PREVIEW_ARG.split("=")[1] : null;
const PREVIEW_FIRST_NAME = process.argv.find((a) => a.startsWith("--preview-first-name="))?.split("=")[1] ?? "Chris";
...
async function main() {
  ...
  if (previewEmail) {
    console.log("[PREVIEW] Sending to the configured preview recipient.");
    eligible = [{ id: "preview", email: previewEmail, first_name: PREVIEW_FIRST_NAME }];
  } else {
    console.log(`[${DRY_RUN ? "DRY-RUN" : "LIVE"}] Fetching contacts from the configured segment…`);
    const contacts = await fetchAllContacts();
    eligible = contacts.filter((c) => c.email && !c.unsubscribed);
    ...
    const payload = await Promise.all(
      batch.map(async (c) => {
        ...
        return {
          ...
          tags: [{ name: "campaign", value: "email4-vote" }],
        };
      })
    );
    try {
